...
namespace Vaettir.Utility
{
	...
	{
		public static IList<string> SplitQuoted(this string value, char delimiter, char quoteChar, char escapeChar, StringSplitOptions options)
		{
			...
			stranged = new Regex($@"\{quoteChar}([^\{quoteChar}]|\{escapeChar}.)*\{quoteChar}").Replace(stranged, m => new String('\0', m.Length));
